...
export default function DeviceModeGate() {
  ...
  if (flags.scan.adapter_lock) {
    if (flags.scan.device_mode === 'skorpio-x5' && !['datalogic-sdk','datalogic-intent','wedge'].includes(activeAdapter)) {
      return (
        <View style={styles.block} testID="adapter-missing">
          <Text style={styles.title}>Hardware scanner not bound</Text>
          <Text style={styles.subtitle}>Please enable Datalogic SDK or Intent in Skorpio settings.</Text>
        </View>
      );
    }
